...
- **GIF animation**: `/api/xsect_gif` generates animated GIF with Pillow (`disposal=2` for Discord). 4 speed options (0.25x/0.5x/0.75x/1x). All loaded FHRs included (up to 19 frames). Terrain + pressure levels locked to first frame for consistency
...
- **Admin key system**: `WXSECTION_KEY` env var now only gates downloading (`/api/request_cycle`). Loading, unloading, Load All, and full-frame GIF are open to all users since mmap makes them cheap (~29MB heap per FHR). UI lock icon still present for download access
- **Load All button**: Visible to all users. Loads all available FHRs for current cycle
- **Protected cycles**: Latest 2 init cycles still tracked for eviction priority, but unloading is unrestricted
...
- **Admin key**: `WXSECTION_KEY` env var checked via `check_admin_key()` against `?key=` query param or `X-Admin-Key` header (with `.strip()`). Only gates `/api/request_cycle` (GRIB downloads from NOMADS/AWS — expensive network+disk I/O). Loading, unloading, Load All, and full-frame GIF are open to all users. `/api/check_key` validates key
...
- **Auto-load**: `auto_load_latest()` called every 60s from background rescan thread. Checks ALL available cycles for PRELOAD_FHRS (F00-F18) on disk but not loaded, opens with 2 parallel workers. Skips gracefully if `_loading` mutex is held by preload or load_cycle
